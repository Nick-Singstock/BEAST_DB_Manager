...
class ScreenSurf():
    ...
    def ScreenSurfaces(self,natomsinsphere=50,keep=1,samespeconly=True,ignore=[], print_errors=False):
        '''
        Screen for low energy surfaces with 3-atom histogram heuristic
        
        ScreenSurfaces uses the following parameters:
            natomsinsphere = Increases radius of sphere until there are in the sphere. For simple structures this can be as low as 20. 
        ...
        '''
        global verbose
        verbose = print_errors
        print("\n*Running Screening Heuristic")
        liststr,longlist, percentages =self.planesearch(self.bulkstruct,natomsinsphere=natomsinsphere,keep=keep,samespeconly=samespeconly,ignore=ignore, verbose=verbose)
        print(liststr)
        if len(liststr)==0:
        ...
        return index, percentages
    
    def planesearch(self,bulk,natomsinsphere=50,keep=0.8,samespeconly=False,ignore=[],maxmult=9, verbose=False):
        '''
        ...
        *atoms* with the highest symmetry within the unit cell. There is a unit-cell-independent way to do this
        ...
        for site in uniqsites:
            ...
            for ii in range(0,len(POS)):
                ...
                # print(millerindex)
                if (millerindex == 0).all() or (millerindex > 10).any():
                    continue
                found=False
                ...
                if found==False:
                    try:               
                        ...
                        if len(C.shape) > 1: # This checks to see if spggetsymfam() returned two vectors or one.
                            millerindex = C[-1]
                        else:
                            continue
                            # millerindex = C.flatten()
                            # print("does it come from here?" ,millerindex, C)
                    except:
                        if verbose: 
                            print("Failed due to irreducible fractional indices")
                        continue
                    # millerindex=C[-1]   ###### Changed this. Not sure if right
                    strm=self.indextostr(millerindex)
                    strsymfam=[]
                    for x in C:
            ...
            sorted_millercheck = sorted(millercheck.items(), key=operator.itemgetter(1),reverse=True)
        ...
        sorttotal=sorted(totaldict.items(), key=operator.itemgetter(1),reverse=True)
        ...
        for ind in sorttotal:
            ...
            if cdf < total*keep:
                if short==True and mult<=maxmult:
                    ...
                    percentages.append(ind[1]/alltotal)
            else:
                break
        return keeplist,sorttotal, percentages
    ...
    def getmillerfrom2v(self,basis,v1,v2,errfrac=False):
        ...
        MLCM=1
        for ii in md:
            MLCM=MLCM*ii / math.gcd(int(MLCM),int(ii[0]))
        ...
        MGCD=np.abs(math.gcd(math.gcd(int(millerindex[0]),int(millerindex[1])),int(millerindex[2])))
    ...
    def spggetsymfam(self,structure, mindex):
        ...
        for op in symmops:
            for site in structure:
                ...
                millerindex=np.array(self.getmillerfrom2v(basis, newv1, newv2, errfrac=True))
                if (millerindex == 0).all():
                    continue
                exists=False
                for Ccheck in C:
                    c=0
    ...
    def get2vectsinplane(self,basis, maxindex):
        ...
        if numzero == 0:
            ...
            multfact=1
            for ii in lst:
                multfact=multfact*ii / math.gcd(int(multfact),int(ii))
        ...
        twovects[0]=twovects[0]/math.gcd(math.gcd(int(twovects[0,0]), int(twovects[0,1])),int(twovects[0,2]))
        twovects[1]=twovects[1]/math.gcd(math.gcd(int(twovects[1,0]), int(twovects[1,1])),int(twovects[1,2]))
